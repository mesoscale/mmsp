...
template <class T>
Point<T> operator+(const Point<T>& p, const Point<T>& q){return Point<T>(p.x+q.x, p.y+q.y, p.z+q.z);}
...
Point<U> operator*(const U& f, const Point<T>& p){return Point<U>(f*p.x, f*p.y, f*p.z);}

template <class T>
Point<T> operator/(const Point<T>& p, const Point<T>& q){return Point<T>(p.x/q.x, p.y/q.y, p.z/q.z);}

template <class T, class U>
Point<T> operator/(const Point<T>& p, const U& f){return Point<T>(p.x/f, p.y/f, p.z/f);}
