	...
	for (int step=0; step<steps; step++) {
        ...
        while (residual>tolerance) {
    		...
    		for (int n=0; n<nodes(grid); n++) {
	    		...
	    		const double denom = A11*A22 - A12*A21;
	    		guess(x)[0] =( B1*A22 -  B2*A12)/denom;
	    		guess(x)[1] =(A11*B2  - A21*B1 )/denom;
             }
    		...
    		for (int n=0; n<nodes(grid); n++) {
	    		...
	    		const double A11 = 1.0;                                      const double A12 = dt*D*lapWeight;
	    		const double A21 = -K*lapWeight - 4.0*AA*pow(guess(x)[0],2); const double A22 = 1.0;

                const double AX1 = A11*guess(x)[0] + A12*guess(x)[1];
                const double AX2 = A21*guess(x)[0] + A22*guess(x)[1];
                const double normBminusAX = std::sqrt(pow(B1-AX1,2) + pow(B2-AX2,2));
                //const double normB = std::sqrt(pow(B1,2)+pow(B2,2));
                const double normB = std::sqrt(pow(update(x)[0],2) + pow(expansive_dfdc(update(x)[0]),2));
