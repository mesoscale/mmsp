...
public:
  ...
  {
    #ifdef MPI_VERSION
    unsigned long max_buff(0);
    unsigned long min_buff(-1);
    int np = MPI::COMM_WORLD.Get_size();
    int id = MPI::COMM_WORLD.Get_rank();
    for (int i=0; i<dim; i++) {
      if (1) {
        ...
        // Small data transfer: blocking Sendrecv should scale
        MPI::COMM_WORLD.Sendrecv(&send_size,1,MPI_UNSIGNED_LONG,send_proc,100,&recv_size,1,MPI_UNSIGNED_LONG,recv_proc,100);
        ...
        if (buff_size > max_buff) max_buff = buff_size;
        if (buff_size < min_buff) min_buff = buff_size;

        // Large data transfer: convert to non-blocking communication
        MPI::Request requests[2];
        requests[0] = MPI::COMM_WORLD.Isend(send_buffer,send_size,MPI_CHAR,send_proc,200);
        requests[1] = MPI::COMM_WORLD.Irecv(recv_buffer,recv_size,MPI_CHAR,recv_proc,200);
      ...
      if (1) {
        ...
        // Small data transfer: blocking Sendrecv should scale
        MPI::COMM_WORLD.Sendrecv(&send_size,1,MPI_UNSIGNED_LONG,send_proc,300,&recv_size,1,MPI_UNSIGNED_LONG,recv_proc,300);
        char* send_buffer = new char[send_size];
        char* recv_buffer = new char[recv_size];
        unsigned long buff_size = this->to_buffer(send_buffer,send_min,send_max);
        if (buff_size > max_buff) max_buff = buff_size;
        ...
        // Large data transfer: convert to non-blocking communication
        MPI::Request requests[2];
        requests[0] = MPI::COMM_WORLD.Isend(send_buffer,send_size,MPI_CHAR,send_proc,400);
        requests[1] = MPI::COMM_WORLD.Irecv(recv_buffer,recv_size,MPI_CHAR,recv_proc,400);
        MPI::Request::Waitall(2,requests);
        MPI::COMM_WORLD.Barrier();
        this->from_buffer(recv_buffer,recv_min,recv_max);
        delete [] send_buffer;
  ...
  friend int N0(const grid& GRID, int i) {return GRID.n0[i];}
  friend int N1(const grid& GRID, int i) {return GRID.n1[i];}
