// mmsp2vti.cpp
// Convert MMSP grid data to XYZ+grainID for scalar integer data type
// Yixuan Tan write this based on mmsp2vti.cpp by Jason Gruber
...
//						output << colors[GRID(k).grain_id()%est_grains] << " ";
            output << GRID(k).grain_id() << " ";  //no color randomization
				} else if (dim == 3) {
...
//						output << colors[GRID(k).grain_id()%est_grains] << " ";
			...
			if (int_type) {
				if (dim == 1) {
					...
					GRID.from_buffer(buffer);
					for (int k = 0; k < MMSP::nodes(GRID); k++)
            output << GRID(k) << " ";  //no color randomization
				} else if (dim == 2) {
