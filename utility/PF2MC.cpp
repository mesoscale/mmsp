// PF2MC.cpp
// Convert MMSP PFgrid data format to MCgrid data format
...
int main(int argc, char* argv[])
{
	...
	if (std::string(argv[1])=="--help") {
		std::cout<<argv[0]<<": convert MMSP PFgrid data format to MCgrid data format.\n";
		std::cout<<"Usage: "<<argv[0]<<" [--help] infile [outfile]\n\n";
	...
	if (dim==1) {
		...
		for (int x=x0; x<x1; x++) {
			int index = 0;
			double max = grid1[x][0];
			for (int i=0; i<fields; i++) {
				double value = grid1[x][i];
				if (value>max) {
					index = i;
					max = value;
				}
	...
	if (dim==2) {
		...
		for (int x=x0; x<x1; x++)
			for (int y=y0; y<y1; y++) {
				int index = 0;
				double max = grid1[x][y][0];
				for (int i=0; i<fields; i++) {
					double value = grid1[x][y][i];
					if (value>max) {
						index = i;
						max = value;
					}
	...
	if (dim==3) {
		...
		for (int x=x0; x<x1; x++)
			for (int y=y0; y<y1; y++)
				for (int z=z0; z<z1; z++) {
					int index = 0;
					double max = grid1[x][y][z][0];
					for (int i=0; i<fields; i++) {
						double value = grid1[x][y][z][i];
						if (value>max) {
							index = i;
							max = value;
						}
